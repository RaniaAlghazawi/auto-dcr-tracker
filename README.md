...
- **Project e-mails → new entry.** Save a project's e-mails (`.msg` from Outlook, or `.eml`) in a folder
  named after the project number: `resources/<project number>/`. The folder can also sit deeper (for
  example `resources/SHOWCASE/20241189/`), and one folder may cover several numbers
  (`resources/…/50137645, 50137647/`). In **New entry**, type the project number and click
  **Read e-mails**. Claude (`claude-opus-5`, structured output, server-side refusal fallback) reads every
  e-mail in the folder and fills the form. You check the fields, and **Save entry** writes the row to
  Excel with the e-mails linked. Auto-replies and duplicate copies are skipped. API:
  `GET /api/projects`, `POST /api/projects/{number}/read`, `POST /api/dcrs`.
...
- **Synthetic test e-mails**: `resources/sample_emails/` holds 20 made-up e-mails (generated by
  `python scripts/make_sample_emails.py`). The project lookup ignores this folder, because the e-mails
  reuse real project numbers.
