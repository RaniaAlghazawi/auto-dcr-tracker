...
- **From project (wizard).** Save a project's e-mails (Outlook `.msg`) in a folder named after the
  project number, directly in `resources/` (e.g. `resources/20251234/`). Click **From project**, enter or
  pick the number, and the wizard (`backend/app/routers/wizard.py`, `frontend/src/pages/Wizard.tsx`)
  has Claude (`claude-opus-5`, structured output, server-side refusal fallback) read every e-mail in the
  folder and fill a **draft**. You check it and click **Add to Excel tracker**, which assigns the next DCR
  number and writes the row with the e-mails linked. The wizard warns when a project already has a DCR,
  offers a blank draft when there are no e-mails, and says so when the e-mails show no quality issue.
  **Manual entry** is the empty form.
...
- **Synthetic test e-mails**: `resources/sample_emails/` holds 20 made-up e-mails (generated by
  `python scripts/make_sample_emails.py`). The wizard ignores this folder.
