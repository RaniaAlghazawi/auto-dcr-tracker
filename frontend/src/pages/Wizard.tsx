...
import { api, type WizardProject } from '../api'
import { BackLink } from '../ui'

export function Wizard({ onCancel, onCreated }: { onCancel: () => void; onCreated: (id: string) => void }) {
  const [suggestions, setSuggestions] = useState<WizardProject[]>([])
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (response.result === 'started') {
        ...
        window.location.hash = `#wizard/working/${encodeURIComponent(project.trim())}`
      } else if ((response.result === 'created' || response.result === 'drafted') && response.id) {
...
export function WizardWorking({ project, onCancel, onDone, onNotADCR }: {
  project: string
  onCancel: () => void
  onDone: (dcrId: string) => void
  onNotADCR: () => void
}) {
  ...
  useEffect(() => {
    ...
    // Poll for status; the backend's next_url is "/detail/<id>" or "/wizard/not-a-dcr/<project>"
    const interval = setInterval(() => {
      api.wizardWorkingStatus(project).then(data => {
        ...
        const detail = /^\/detail\/(.+)$/.exec(data.next_url || '')
        if (detail) {
          clearInterval(interval)
          onDone(detail[1])
        } else if (data.outcome === 'not_a_dcr') {
          clearInterval(interval)
          onNotADCR()
        } else if (data.outcome === 'error') {
          clearInterval(interval)
          setFailure(data.reason || 'Reading the e-mails failed.')
        }
      }).catch(console.error)
    }, 2000)
    ...
    <section className="view">
      ...
      <div className="card">
        ...
        <div style={{ padding: 20, textAlign: 'center' }}>
          ...
          {failure && <div className="notice is-critical" style={{ marginBottom: 12 }}>{failure}</div>}
